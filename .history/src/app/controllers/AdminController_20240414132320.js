  ...
  editWarranty(req, res, next) {
    Warranty.findById(req.params.id)
      ...
      .then((warranty) => {
        ...
        // Giả định productsAndReasons là một array có cả product và reason
        productsAndReasons.forEach(function (element) {
          Product.find({ _id: element.product }).then((product) => {
            // Nếu tìm thấy sản phẩm
            if (product) {
              product._doc.selected = true;
              product._doc.uniqueProductName = product.name; //Thêm thuộc tính uniqueProductName vào đối tượng sản phẩm
            }
          });
        });
