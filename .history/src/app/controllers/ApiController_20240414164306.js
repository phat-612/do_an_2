...
class ApiController {
  ...
  updateWarranty(req, res, next) {
    ...
    productDetails.forEach((detail) => {
        const productId = detail.idProduct;
        const reasonsAndPrices = detail.reasonsAndPrices;
        // update product info
        Product.findByIdAndUpdate(productId, 
            {name: detail.productName}, // update the name, or any other fields you'd like
            (err, product) => {
                // handle error
                if(err) {
                    console.log(err);
        ...
        // update reasons and prices
        reasonsAndPrices.forEach((reasonAndPrice) => {
            ReasonAndPrice.findByIdAndUpdate(reasonAndPrice._id, 
                {reason: reasonAndPrice.reason, price: reasonAndPrice.price}, // update reason and price
                (err, result) => {
                    // handle error
                    if(err) {
                        console.log(err);
