...
class ApiController {
  ...
  updateWarranty(req, res, next) {
    Warranty.findOne({ _id: req.params.id }).then((warranty) => {
      ...
      // Tìm chỉ mục (index) của sản phẩm cần cập nhật trong mảng 'details'
      const index = warranty.details.findIndex((detail) =>
        detail.idProduct.equals(req.body.idProduct)
      );

      // Nếu tìm được sản phẩm trong mảng 'details'
      if (index !== -1) {
        // Cập nhật 'reasonAndPrice' của sản phẩm
        warranty.details[index].reasonAndPrice = req.body.reasonAndPrice;
      } else {
        // Nếu không tìm được sản phẩm, thêm mới vào mảng 'details'
        const newDetail = {
          idProduct: req.body.idProduct,
          reasonAndPrice: req.body.reasonAndPrice,
        };
        warranty.details.push(newDetail);
      }
