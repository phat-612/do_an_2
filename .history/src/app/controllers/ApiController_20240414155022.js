...
class ApiController {
  ...
  storeWarranty(req, res, next) {
   ...
   updateWarranty(req, res, next) {
    Warranty.findById(req.body.warrantyId, function(err, warranty) {
      if (!err) {
        // Tìm detail bằng cách sử dụng idProduct trong mảng details
        const detail = warranty.details.find(detail => detail.idProduct.toString() === req.body.productID);
        if (detail) {
          // cập nhật reason và price
          detail.reasonAndPrice.push({reason: req.body.reason, price: req.body.price});
          warranty.save();
          res.send("cập nhật thành công");
        } else {
          // nếu không tìm thấy detail, có thể tạo mới
          res.send("không tìm thấy sản phẩm")
        }
      } else {
        res.send("không tìm thấy bảo hành");
      }
    });
  });
  }
