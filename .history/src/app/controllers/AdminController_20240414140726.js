  ...
  editWarranty(req, res, next) {
    Warranty.findById(req.params.id)
      ...
      .then((warranty) => {
       ...
       // Copy mảng products
let combinedProducts = JSON.parse(JSON.stringify(products));

// Duyệt sản phẩm trong combinedProducts
combinedProducts.forEach(product => {
  // Kiểm tra xem sản phẩm có tồn tại trong foundProducts không, nếu có thì đánh dấu là đã chọn (selected).
  product.selected = !!foundProducts.find(foundProduct => foundProduct.name === product.name);
});

// Cập nhật đoạn mã res.render để truyền combinedProducts thay vì products và foundProducts.
res.render("admin/warrantys/edit-warranty", {
  layout: "admin",
  warranty: mongooseToObject(warranty),
  productsAndReasons,
  products: multipleMongooseToObject(combinedProducts),
});
  })
}
