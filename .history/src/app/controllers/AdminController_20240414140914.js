          ...
          products.forEach((product) => {
            if (product && product._doc) {
              let productFound = productsAndReasons.find(
                (productAndReason) =>
                  productAndReason.productName === product.name
              );
              // Nếu sản phẩm tìm thấy trong 'productsAndReasons'
              if (productFound) {
                product._doc.selected = true;
                product._doc.uniqueProductName = product.name;
                foundProducts.push(product._doc);
              } else {
                console.error(
                  `Sản phẩm ${product.name} không tìm thấy trong 'productsAndReasons'`
                );
              }
            }
          });
        
        // Sau đây là phần cần thêm:
          let combinedProducts = products.map(product => {
            let found = foundProducts.some(foundProduct => foundProduct.name === product.name);
            return {
              ...product._doc,
              selected: found
            }
          ...
          res.render("admin/warrantys/edit-warranty", {
            ...
            warranty: mongooseToObject(warranty),
            productsAndReasons,
            products: multipleMongooseToObject(combinedProducts), // Sử dụng combinedProducts
          });
        });
  }
}
