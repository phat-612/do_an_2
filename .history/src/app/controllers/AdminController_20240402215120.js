  ...
  detailWarranty(req, res, next) {
    ...
    .then((warranty) => {
      const productNames = warranty.details.map((detail) =>
        detail.idProduct && detail.idProduct.name ? detail.idProduct.name : null
      );
  
      const reasonsAndPrices = warranty.details
        .flatMap((detail) =>
          detail.reasonAndPrice
            ? detail.reasonAndPrice.filter(
                (reasonPrice) => reasonPrice.reason && reasonPrice.price
              )
            : []
        )
        .map((reasonPrice) => ({
      ...
      warranty.details.forEach((detail) => {
        ...
        const reasonsAndPrices = detail.reasonAndPrice
          ...
          : [];
        reasonsAndPrices.forEach((reasonPrice) => {
      ...
      res.render("admin/warrantys/detail-warranty", {
        ...
        productNames,
        reasonsAndPrices,
        productsAndReasons: mongooseToObject(productsAndReasons),
      });
    })
    .catch((error) => {
      // Xử lý lỗi nếu có
