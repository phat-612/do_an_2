  ...
  detailWarranty(req, res, next) {
    Warranty.findById(req.params.id)
      ...
      .then((warranty) => {
        ...
        const detailData = warranty.details.map((detail) => ({
          product: detail.idProduct ? detail.idProduct.name : "Không xác định",
          reasonAndPrice: detail.reasonAndPrice.map(({ reason, price }) => ({
            reason,
            price,
          })),
        }));
