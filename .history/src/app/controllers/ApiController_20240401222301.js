...
class ApiController {
  ...
  storeWarranty(req, res, next) {
    ...
    if (
      warrantyDataArray &&
      Array.isArray(warrantyDataArray) &&
      warrantyDataArray.length > 0
    ) {
      uniqueDataArray = warrantyDataArray.filter((data, index, self) => {
        if (
          data.details &&
    ...
    uniqueDataArray.forEach((data) => {
      ...
      const promise = Warranty.findOne({ "details.idProduct": data.details[0].idProduct })
        .then((existingWarranty) => {
          if (!existingWarranty) {
            // Bản ghi không tồn tại, lưu vào CSDL
            return warranty.save();
          }
          return null; // Trả về null nếu bản ghi đã tồn tại
        });
      ...
      .then(() => {
        ...
        Warranty.insertMany(uniqueDataArray, { ordered: false })
          ...
          .catch((error) => {
            // Xử lý lỗi nếu có
            console.error(error);
            res.redirect("/admin/warranty/error");
          });
      })
      .catch((error) => {
        // Xử lý lỗi nếu có
