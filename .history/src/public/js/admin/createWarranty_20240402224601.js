...
form.addEventListener("submit", function (event) {
  ...
  for (var i = 0; i < productContainers.length; i++) {
    ...
    if (isAddClicked !== "true") {
      alert("Bạn phải thêm chi tiết cho mỗi sản phẩm");
...
// Thêm đoạn mã sau để thêm/xóa thuộc tính "required" cho trường "idsanpham"
inputSanPham.addEventListener("input", function () {
  if (inputSanPham.value.trim() === "") {
    inputSanPham.setAttribute("required", "required");
  } else {
    inputSanPham.removeAttribute("required");
  }
});
