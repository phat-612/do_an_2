...
function createAttri1Row() {
  ...
  // Tạo hàng cho mỗi giá trị thuộc tính
  for (let i = 0; i < Math.max(attr1Values.length, attr2Values.length); i++) {
    var newRow = document.createElement("tr");

    // Tạo cell cho thuộc tính 1
    var td1 = document.createElement("td");
    td1.className = "td1";
    if (i < attr1Values.length) {
      td1.textContent = attr1Values[i];
    }
    newRow.appendChild(td1);

    // Tạo cell cho thuộc tính 2
    var td2 = document.createElement("td");
    td2.className = "td2";
    if (i < attr2Values.length) {
      td2.textContent = attr2Values[i];
    }
    newRow.appendChild(td2);

    // Tạo cell cho các cột giá và số lượng (giả sử mặc định là 1300 và 1200)
    var tdPrice = document.createElement("td");
    tdPrice.textContent = "1300";
    newRow.appendChild(tdPrice);

    var tdQuantity = document.createElement("td");
    tdQuantity.textContent = "1200";
  ...
  // Xóa nội dung cũ của tbody và thêm tbody mới vào bảng
  var table = document.getElementById("custom-varriant");
