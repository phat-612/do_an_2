...
addProductButton.addEventListener("click", function () {
  ...
  for (var i = 0; i < options.length; i++) {
    if (!options[i]) {
      console.log("options[" + i + "] does not exist");
      break;
    } else if (options[i].innerText === selectedOption) {
      if (!options[i].idProduct) {
        console.log("options[" + i + "] does not have a property idProduct");
        break;
      }

      selectedProductId = options[i].idProduct;
      break;
    }
  }
  ...
  if (selectedOption !== "" && selectedProductId !== "") {
    ...
    inputSanPham.value = "";
    productCounter++;
    inputSanPham.required = false; // Thêm lại thuộc tínhrequired cho ô input
  }
});
